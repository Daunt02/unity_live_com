...
STATIC_ROOT = '/home/%s/webapps/%s/static/' % (USER, APP_NAME)
...
PYTHON_FULL_PATH = "%s/bin/%s" % (PYTHON_PREFIX,
                                  PYTHON_BIN) if PYTHON_PREFIX else PYTHON_BIN
...
def install_dependencies():
    ...
    with virtualenv(VENV_DIR):
        with cd(SRC_DIR):
            run_venv("pip install -r .requirements")
...
def push_sources():
    ...
    if push_rev is None:
        push_rev = local(
            "git id",
            capture=True).split(" ")[0].strip().strip("+")

    local("git push -f ssh://%(user)s@%(host)s/%(path)s || true" %
          dict(host=env.host,
               user=env.user,
               path=SRC_DIR,
               ))
    with cd(SRC_DIR):
        run("git update %s" % push_rev)
